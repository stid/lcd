...
7. **Calibration read (outcome telemetry).** The log carries two line shapes — triage lines (`… · n signals · lane · hard · risk`) and closeout lines (`… · closeout · lane · audit · re-routes [· red-green iters] · interventions`; the `red-green iters` field is optional since 0.18.0 — read lines with and without it). Cross-read them and flag:
