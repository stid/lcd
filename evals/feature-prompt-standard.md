...
> do not edit between runs of one experiment. Derived from `feature-prompt.md` with ONE
> delta (work-item lean-loop): the LCD preamble routes through triage instead of
> mandating a lane — it never names an expected lane, so the run measures what triage
> actually does with the task and what the routed lane costs.
